...
namespace Bot.Tools {
  public class ErrorableFactoryTryCatchDecorator<T, TOutput> : IErrorableFactory<T, TOutput> {
    private readonly IErrorableFactory<T, TOutput> _factory;
    private readonly ILogger _logger;

    public ErrorableFactoryTryCatchDecorator(IErrorableFactory<T, TOutput> factory, ILogger logger) {
      _factory = factory;
      _logger = logger;
  ...
  public class ErrorableFactoryTryCatchDecorator<T1, T2, TOutput> : IErrorableFactory<T1, T2, TOutput> {
    private readonly IErrorableFactory<T1, T2, TOutput> _factory;
    private readonly ILogger _logger;

    public ErrorableFactoryTryCatchDecorator(IErrorableFactory<T1, T2, TOutput> factory, ILogger logger) {
      _factory = factory;
      _logger = logger;
