...
namespace Bot.Logic {
  public class NukeFactory : IFactory<IReceived<Moderator, IMessage>, ParsedNuke> {
    ...
    public ParsedNuke Create(IReceived<Moderator, IMessage> message) {
      if (message.IsMatch(_modCommandRegex.Nuke)) {
        ...
        return new ParsedNuke(message.Timestamp, duration, matchesNukedTerm);
